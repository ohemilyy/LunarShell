...
SERVICES_DOWN_ICON=""
SERVICES_DOWN_COLOR="1;33"
SERVICES_FILE="/tmp/dragon_services"
...
print_banner() {
    printf "    \\033[1;37mSystem Information:\\033[0m\\n"
    if [ -f /etc/os-release ]; then
	    ...
	    banner_distro_version="$VERSION_ID"
            ...
            banner_distro_icon="$(echo -e "\xef\x8c\x96\x0a")"
            banner_distro_color="1;32"
            banner_distro_name="Rocky"
            banner_distro_version=$VERSION_ID
        elif [ "$ID_LIKE" = "rhel centos fedora" ]; then
            banner_distro_icon="$(echo -e "\xef\x8c\x96\x0a")"
            banner_distro_color="1;31"
            banner_distro_name="EL"
            banner_distro_version=$VERSION_ID
        else
            banner_distro_icon="?"
